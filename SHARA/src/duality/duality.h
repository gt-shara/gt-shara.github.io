...
namespace Duality {
    ...
    RPFP(LogicSolver *_ls) : Z3User(*(_ls->ctx)), dualModel(*(_ls->ctx)), aux_solver(_ls->aux_solver)
        ...
        Node(const FuncDecl &_Name, const Transformer &_Annotation, const Transformer &_Bound, const Transformer &_Underapprox, const Term &_dual, RPFP *_owner, int _number,int _TopoSort)
            : Name(_Name), Annotation(_Annotation), Bound(_Bound), Underapprox(_Underapprox), dual(_dual) {owner = _owner; number = _number; recursion_bound = UINT_MAX;level=0; TopoSort=_TopoSort;}
        };
        ...
        void DeleteNode(Node *node){
            if(!node->Outgoing.empty() || !node->Incoming.empty())
        ...
        Edge *CreateEdge(Node *_Parent, const Transformer &_F, const std::vector<Node *> &_Children)
        {
            ...
            _Parent->Outgoing.push_back(e);
           for(unsigned i = 0; i < _Children.size(); i++){
               if(std::find(_Children[i]->Incoming.begin(), _Children[i]->Incoming.end(), e) == _Children[i]->Incoming.end()){
        ...
        void DeleteEdge(Edge *edge){
            if(edge->Parent){
                edge->Parent->Outgoing.erase(edge->Parent->Outgoing.begin());
	...
	: name(s), value(e), pos(b) {}
        ...
        */
        ...
        static std::set<Node*> collectDependencies(std::set<Node*> roots);

        static std::set<Node*> collectDependents(std::set<Node*> leaves);
        ...
        */
        ...
        Term Localize(Edge *e, const Term &t,bool ifShara=false);
        ...
        */
        ...
        void SetEdgeMaps(Edge *e,bool ifShara=false);
        
        void setReplaceVars(Node *node,hash_map<ast,Term> &memo);
        
        RPFP::Term getNodeConstrains(Node *node);
        
        void RedVars(Node *node, Term &b, std::vector<Term> &v,bool ifShara=false);
    ...
    protected:
        ...
        Term SuffixVariable(const Term &t, int n,int i);
      
        Term HideVariable(const Term &t, int n);


        Term RedDualRela(Edge *e, std::vector<Term> &args, int idx,bool ifShara=false);

        Term LocalizeRec(Edge *e,  hash_map<ast,Term> &memo, const Term &t,bool ifShara=false);

        Term ReducedDualEdge(Edge *e,bool ifShara=false);
    ...
    public:
        ...
        void SetAnnotation(Node *root, const expr &t,bool ifShara = false);
