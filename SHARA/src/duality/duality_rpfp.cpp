...
namespace Duality {
    ...
    RPFP::Term RPFP::SuffixVariable(const Term &t, int n,int i)
    {   std::string name = t.decl().name().str() + "_" + string_of_int(n);
        return ctx.constant(name, t.get_sort());
    }
    ...
    void RPFP::setReplaceVars(Node *node,hash_map<ast,Term> &memo){
        int idx= node->number;
        std::vector<Term> v=node->Annotation.IndParams;
        for(unsigned i=0; i<v.size();i++){
            memo[v[i]]=SuffixVariable(v[i],idx,i);
        }
    
    }
    
    RPFP::Term RPFP::getNodeConstrains(Node *node){
        hash_map<ast,Term> memo;
        setReplaceVars(node,memo);
        Term annot = SubstRec(memo, node->Annotation.Formula);
        return annot;
    }
    
    void RPFP::RedVars(Node *node, Term &b, std::vector<Term> &v,bool ifShara)
    {
        int idx = node->number;
        if(ifShara)
            b = ctx.bool_val(true);
    ...
    RPFP::Term RPFP::RedDualRela(Edge *e, std::vector<Term> &args, int idx,bool ifShara){
        Node *child = e->Children[idx];
        Term b(ctx);
        std::vector<Term> v;
        RedVars(child, b, v,ifShara);
  ...
  // collectDependencies: get the dependencies of a given vector of nodes
  std::set<RPFP::Node*> RPFP::collectDependencies(std::set<RPFP::Node*> roots) {
    std::set<Node*> deps;
  ...
  // collectDependents: get the dependents of a set of nodes
  std::set<RPFP::Node*> RPFP::collectDependents(std::set<RPFP::Node*> leaves) {
    std::set<Node*> deps;
    std::set<Node*> worklist = leaves;
   ...
   // replace with: = worklist.is_empty() in loop;
    ...
    RPFP::Term RPFP::LocalizeRec(Edge *e,  hash_map<ast,Term> &memo, const Term &t,bool ifShara)
    {
        ...
        if (t.is_app()) {
            ...
            int nargs = t.num_args();
            for (int i = 0; i < nargs; i++)
                args.push_back(LocalizeRec(e, memo, t.arg(i),ifShara));
            hash_map<func_decl, int>::iterator rit = e->relMap.find(f);
            if (rit != e->relMap.end())
                res = RedDualRela(e, args, (rit->second),ifShara);
            else {
            ...
            for (unsigned i = 0; i < pats.size(); i++)
                pats[i] = LocalizeRec(e, memo, pats[i],ifShara);
            Term body = LocalizeRec(e, memo, t.body(),ifShara);
    ...
    void RPFP::SetEdgeMaps(Edge *e,bool ifShara){
    ...
    RPFP::Term RPFP::Localize(Edge *e, const Term &t,bool ifShara){
        timer_start("Localize");
        hash_map<ast,Term> memo;
        if(e->F.IndParams.size() > 0 && e->varMap.empty())
            SetEdgeMaps(e,ifShara); // TODO: why is this needed?
        Term res = LocalizeRec(e,memo,t,ifShara);
        timer_stop("Localize");
        return res;
    }


    RPFP::Term RPFP::ReducedDualEdge(Edge *e,bool ifShara)
    {
        SetEdgeMaps(e);
        timer_start("RedVars");
        Term b(ctx);
    ...
    TermTree *RPFP::ToTermTree(Node *root, Node *skip_descendant)
    {
        ...
        if(root->Outgoing.empty()) return new TermTree(ctx.bool_val(true), std::vector<TermTree *>());
        Edge *e = root->Outgoing.at(0);
        ...
        for(unsigned i = 0; i < e->constraints.size(); i++){
            ...
            std::cout<<"the constrains is:\n";
            std::cout<<e->constraints[i]<<"\n";
        }
        return res;
    }
    ...
    TermTree *RPFP::ToGoalTree(Node *root)
    {
        ...
        if(root->Outgoing.empty()) return new TermTree(goal, std::vector<TermTree *>());
        Edge *e = root->Outgoing.at(0);
        std::vector<TermTree *> children(e->Children.size());
        for(unsigned i = 0; i < children.size(); i++)
    ...
    void RPFP::SetAnnotation(Node *root, const expr &t,bool ifShara){
        hash_map<ast, Term> memo;
        Term b;
        std::vector<Term> v;
        RedVars(root, b, v,ifShara);
    ...
    void RPFP::DecodeTree(Node *root, TermTree *interp, int persist) {
        ...
        if (ic.size() > 0) {
            std::vector<Node *> &nc = root->Outgoing[0]->Children;
    ...
    expr RPFP::GetEdgeFormula(Edge *e, int persist, bool with_children, bool underapprox) {
        if (e->dual.null()) {
            ...
            if (underapprox) {
                std::cout<<"this is being triggered\n";
    ...
    lbool RPFP::SolveSingleNode(Node *root, Node *node)
    {
        ...
        std::cout<<root->Name.name()<<root->number<<"\n";
        std::cout<<"the node is:\n";
        std::cout<<node->Name.name()<<node->number<<"\n"; */
        timer_start("Solve");
        ...
        timer_start("interpolate_tree");
        /* if(dualModel==NULL){
            std::cout<<"dualModel is null\n";
        } */
        /* std::cout<<"The term tree solve\n";
        std::cout<<"first level\n";
        std::cout<<tree->getTerm()<<"\n";
        std::cout<<"terms\n";
        for(expr theTerm:tree->getTerms()){
            std::cout<<theTerm<<"\n";
        }
        std::cout<<"number of term tree children is "<<tree->getChildren().size();*/
        ...
        if (!underapproxes.size())
            ...
            if (underapprox_core) {
                ...
                if (dump) {
                    ...
                    cnsts.push_back(root->Outgoing[0]->dual);
    ...
    */
    ...
    RPFP::Term RPFP::UnderapproxFormula(const Term &f, hash_set<ast> &dont_cares){
        ...
        std::cout<<"In underapproxFormula\n";
        std::cout<<f<<"\n";
        hash_map<ast,int> memo;
#ifdef Z3OPS
    ...
    */
    ...
    RPFP::Term RPFP::ComputeUnderapprox(Node *root, int persist){
        ...
        resolve_ite_memo.clear();
        timer_start("UnderapproxFormula");
        Term dual = root->Outgoing[0]->dual.null() ? ctx.bool_val(true) : root->Outgoing[0]->dual;
    ...
    */
    ...
    void RPFP::InterpolateByCases(Node *root, Node *node){
        ...
        is.push();
        AddEdgeToSolver(is,node->Outgoing[0]);
        node->Annotation.SetEmpty();
        hash_set<ast> *core = new hash_set<ast>;
        core->insert(node->Outgoing[0]->dual);
        ...
        while(1){
            ...
            prev_impl = the_impl;
            ConstrainEdgeLocalized(node->Outgoing[0],the_impl);
            ConstrainEdgeLocalized(node->Outgoing[0],!GetAnnotation(node)); //TODO: need this?
      
            {
                ...
                try {
                    std::cout<<"what is the cases here\n";
...
#define TEST_BAD
    ...
    void RPFP::Generalize(Node *root, Node *node){
        ...
        aux_solver.push();
        AddEdgeToSolver(node->Outgoing[0]);
...
#define USE_QE_LITE
    ...
    void RPFP::WriteCounterexample(std::ostream &s, Node *node){
        for(unsigned i = 0; i < node->Outgoing[0]->Children.size(); i++){
            Node *child = node->Outgoing[0]->Children[i];
            if(!Empty(child))
        ...
        for(unsigned i = 0; i < node->Outgoing[0]->Children.size(); i++){
            Node *child = node->Outgoing[0]->Children[i];
            if(!Empty(child))
                s << " " << node->Outgoing[0]->Children[i]->number;
        }
        s << ")" << std::endl;
        WriteEdgeAssignment(s,node->Outgoing[0]);
    }
